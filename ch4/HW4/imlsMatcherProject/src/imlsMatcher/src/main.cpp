...
// 选择的方法 1/2  (1: IMLS-ICP    2: PL-ICP in CSM library)
int m_icp_methode;
...
public:
    ...
    void championLaserScanCallback(const champion_nav_msgs::ChampionNavLaserScanConstPtr& msg)
    {
        ...
        if(m_icp_methode == 2){
            ...
            d_point_scan = m_plicpMatcher.PLICPBetweenTwoFrames(currentLDP, Eigen::Vector3d(0,0,0));

            if(m_plicpMatcher.m_OutputResult.valid)
            {
                std::cout << "PL-ICP Match Successful:" << d_point_scan(0) << "," << d_point_scan(1) << ","
                          << d_point_scan(3) * 57.295 << std::endl;
...
int main(int argc, char** argv)
{
    ...
    std::cout << "Please enter the ICP methode you want to use: (1: IMLS-ICP    2: PL-ICP in CSM library)" << endl;
    std::cin >> m_icp_methode;
