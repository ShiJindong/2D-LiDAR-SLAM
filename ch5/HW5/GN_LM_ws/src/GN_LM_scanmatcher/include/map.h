 ...
 * https://blog.csdn.net/bzhxuexi/article/details/45367177
 * typedef struct {...} s1;    这种方法可以在c或者c++中定义一个S1结构。其中，S1为结构体类型
 * struct s2 {...};            这种定义方式只能在C++中使用，而如果用在C中，那么编译器会报错。其中，S2为结构体类型
 * struct {...} s3;            这种方法并没有定义一个结构，而是定义了一个s3的结构变量，编译器会为s3内存。其中，S3为结构体对象。
 */
...
typedef struct
{
  ...
  // Distance to the nearest occupied cell (为实际距离)
  double occ_dist;
...
typedef struct
{
  ...
  // The map data, stored as a grid 每一个栅格
  map_cell_t *cells;
...
// Convert from world coords to map coords 世界坐标转换到地图坐标       // (floor(x)返回的是小于或等于x的最大整数, floor(x+0.5)保证对小数四舍五入取整)
...
#define MAP_GXWX_DOUBLE(map, x) ((x - map->origin_x) / map->resolution + 0.5 + double(map->size_x / 2))    // 返回double类型的地图坐标
