...
int main(int argc, char **argv)
{
    ...
    // std::string VertexPath = "/home/jindong/Lidar_SLAM/Exercise/ch6/HW6/LSSLAMProject/src/ls_slam/data/test_quadrat-v.dat";
    // std::string EdgePath = "/home/jindong/Lidar_SLAM/Exercise/ch6/HW6/LSSLAMProject/src/ls_slam/data/test_quadrat-e.dat";

    std::string VertexPath = "/home/jindong/Lidar_SLAM/Exercise/ch6/HW6/LSSLAMProject/src/ls_slam/data/intel-v.dat";
    std::string EdgePath = "/home/jindong/Lidar_SLAM/Exercise/ch6/HW6/LSSLAMProject/src/ls_slam/data/intel-e.dat";

    // std::string VertexPath = "/home/jindong/Lidar_SLAM/Exercise/ch6/HW6/LSSLAMProject/src/ls_slam/data/killian-v.dat";
    // std::string EdgePath = "/home/jindong/Lidar_SLAM/Exercise/ch6/HW6/LSSLAMProject/src/ls_slam/data/killian-e.dat";
    ...
    int solver_select = 0;
    std::cout << "Which solver would you like to use? enter [1: Eigen, 2: Ceres, 3: g2o]" << std::endl;   
    ...
    if(solver_select == 1)
    {
        ...
        for(int i = 0; i < maxIteration;i++)
        {
            ...
            pcl::console::TicToc t_solver;
            t_solver.tic();
            Eigen::VectorXd dx = LinearizeAndSolve(Vertexs,Edges);
            printf("solver time for one step: %f ms \n", t_solver.toc());
            //进行更新
            //TODO--Start
            for(int j = 0; j != Vertexs.size(); ++j)
                Vertexs[j] += dx.segment(j*3, 3);
            //TODO--End
    ...
    else if(solver_select == 2)
    {
        ...
        for (int i=0; i < Edges.size(); ++i)
        {
            ...
            Edge tmpEdge = Edges[i];
            const Eigen::Matrix3d sqrt_information = tmpEdge.infoMatrix.llt().matrixL();
    ...
    {
        std::cout << "There is no solver for number: " << solver_select << "!"<< std::endl; 
    }


    auto run_end = std::chrono::high_resolution_clock::now();
    std::cout << "solver used all time: " << std::chrono::duration_cast<std::chrono::milliseconds>(run_end - run_start).count() << " ms"<< std::endl; 
